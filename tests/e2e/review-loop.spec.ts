...
test('captures English terms and persists the List after reload', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, 'SpeechSynthesisUtterance', { configurable: true, value: BlockedUtterance });
    Object.defineProperty(window, 'speechSynthesis', {
      configurable: true,
      value: {
        cancel() {},
        speak(utterance: BlockedUtterance) {
  ...
  const recovery = page.getByRole('button', { name: '点击页面开启发音' });
